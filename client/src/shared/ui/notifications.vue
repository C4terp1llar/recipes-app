...
<template>
  <div v-if="ntfStore.notifications.length" class="fixed right-3 max-w-lg ml-3 bottom-3 p-4 flex flex-col gap-4">
    <div
        v-for="notification in ntfStore.notifications"
        :key="notification.id"
        :class="[
        'flex z-10 w-fit ml-auto items-center gap-3 p-2! rounded-lg border shadow-lg transform transition-colors duration-300 bg-white ',
